...
left_join(unemployment_local, by =  "State")
...
local_model_data_w7 <- local_model_data_w7 %>%
left_join(ratings_share_2018, by = "CD")
...
w7_df <-districts_simp %>%
left_join(local_model_data_w7, by = "CD", "district_id")
...
w7_cleaned <- rmapshaper::ms_simplify(w7_df, keep = 0.01)
```

```{r w7 prediction, echo=FALSE}
w7_local_model <- glm(DemVotesMajorPercent.x ~ avg + Unemployed_prct + winner_candidate_inc.x + Receipts + turnout, data = w7_df)
...
My final prediction for Week 7 is that
...
```{r, prediction}
districts22 <- w7_df %>% group_by(CD) %>% filter(row_number() == 1)
